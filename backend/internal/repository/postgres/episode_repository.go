...
func (r *EpisodeRepository) Create(ctx context.Context, episode *entity.Episode) error {
	...
	const query = `
		INSERT INTO episodes (season_id, name, path, fetch_source)
		VALUES ($1, $2, $3, $4)
		RETURNING id, created_at
	`

	err := r.db.QueryRowxContext(ctx, query, episode.SeasonID, episode.Name, episode.Path, fetchSource).Scan(&episode.ID, &episode.CreatedAt)
	if err != nil {
...
func (r *EpisodeRepository) GetByPath(ctx context.Context, path string) (*entity.Episode, error) {
	const query = `
		SELECT id, season_id, name, path, created_at, fetch_source
		FROM episodes
		WHERE path = $1
		LIMIT 1
	`
	...
	if err := r.db.GetContext(ctx, &episode, query, path); err != nil {
