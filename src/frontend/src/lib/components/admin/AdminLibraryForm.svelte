<script lang="ts">
	...
	let { library }: { library: LibraryListItem | null } = $props();
	...
	$effect(() => {
		if (library == null) {
			form.name = '';
			form.type = LibraryType.Shows;
			form.path = '';
		} else {
			form.name = library.name;
			form.type = library.library_type as LibraryType;
...
<h1 class="mb-4 text-2xl tracking-wider">
	{#if library == null}
		New Library
	{:else}
		Update Library
