...
>
	...
	>
		...
		<div class="mb-3 flex items-center gap-3 text-xs text-white/80">
			...
			<div
				bind:this={seekBarEl}
				class="group relative h-2 grow cursor-pointer touch-none rounded-full bg-neutral-500/80 transition-all duration-300"
				role="slider"
				tabindex="0"
				aria-label="Seek"
				aria-valuemin="0"
			...
			>
				<div
					class={`absolute h-full rounded-full bg-neutral-500 transition-all duration-300`}
					style={`width: ${getPercentageBetween(0, seekMax, bufferedUntil)}%;`}
				></div>
				<div
					class={`absolute h-full rounded-full bg-brand transition-all duration-300`}
